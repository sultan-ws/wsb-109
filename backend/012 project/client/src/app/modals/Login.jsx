...
function SignUpBox({ setCompStatus, compStatus }) {
  ...
  const ifValid = () => {
    ...
    if (!user.firstname) errors.name = 'Plaese insret a valid first name';

    if (user.firstname) {
      if (!user.firstname.trim()) errors.name = 'Plaese insret a valid first name';
    }

    if (!user.lastname) errors.lastname = 'Plaese insret a valid last name';

    if (user.lastname) {
      if (!user.lastname.trim()) errors.lastname = 'Plaese insret a valid last name';
    }


    const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

    if (!emailPattern.test(user.email)) errors.email = 'Please insert a valid email';

    const passwordPattern = /^(?=.*\d)(?=.*[a-z])(?=.*[^a-zA-Z0-9])(?!.*\s).{7,15}$/;

    if (!passwordPattern.test(user.password)) errors.password = 'Plaese include Minimum 8 characters, Maximum 16 characters, At least one uppercase character, At least one lowercase character, At least one digit and At least one special character in password';

    setRegexError(errors);
    return Object.keys(errors).length !== 0;

  };

  const handleGeanrateOtp = () => {
    const ifErrors = ifValid();
